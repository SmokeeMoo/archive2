...
class Link {
    ...
    public static function get_biolink($tthis, $link, $user = null, $biolink_blocks = null) {
        ...
        /* Background blur */
        if((isset($link->settings->background_blur) && $link->settings->background_blur != 0) || isset($link->settings->background_brightness) && $link->settings->background_brightness != 100) {
            $link->design->background_style .= 'backdrop-filter: blur(' . $link->settings->background_blur .'px) brightness(' . $link->settings->background_brightness . '%);';
        }
        ...
        switch($link->type) {
            ...
            case 'alert':
                ...
                $link->design->link_style =
                    ...
                    . 'box-shadow: ' . ($link->settings->border_shadow_offset_x ?? '0') . 'px ' . ($link->settings->border_shadow_offset_y ?? '0') . 'px ' . ($link->settings->border_shadow_blur ?? '20') . 'px ' . ($link->settings->border_shadow_spread ?? '0') . 'px ' . ($link->settings->border_shadow_color ?? '#00000010') . ';'
