...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="cardslider" />
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#setting" aria-expanded="false" aria-controls="setting">
		...
		<?= l('create_biolink_cardslider_modal.cardslider_settings') ?>
	...
	<div class="collapse" id="setting">	
		...
		<div class="custom-control custom-switch mb-3">
			...
			>
			<label class="custom-control-label" for="<?= 'slideshow_autoplay' . $row->biolink_block_id ?>"><?= l('create_biolink_cardslider_modal.cardslider_settings_autoplay') ?></label>
		</div>
		...
		<div class="custom-control custom-switch mb-3">
			...
			>
			<label class="custom-control-label" for="<?= 'arrows' . $row->biolink_block_id ?>"><?= l('create_biolink_cardslider_modal.cardslider_settings_arrows') ?></label>
		</div>	
		...
		<div class="custom-control custom-switch mb-3">
			...
			>
			<label class="custom-control-label" for="<?= 'dots' . $row->biolink_block_id ?>"><?= l('create_biolink_cardslider_modal.cardslider_settings_dots') ?></label>
		</div>	
		
		
		 <div class="form-group">
                <label for="<?= 'cardslider_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_cardslider_modal.input.text_color') ?></label>
                <input id="<?= 'cardslider_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                <div class="text_color_pickr"></div>
            </div>
        
                    <div class="form-group">
                <label for="<?= 'cardslider_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_cardslider_modal.input.background_color') ?></label>
                <input id="<?= 'cardslider_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
                <div class="background_color_pickr"></div>
            </div>
		

		<div class="form-group">
			<label for="<?= 'number_of_sliders_for_mobile_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-mobile fa-sm mr-1"></i><?= l('create_biolink_cardslider_modal.slider_settings_mobilescreen') ?></label>
		...
		<div class="form-group">
			<label for="<?= 'number_of_sliders_for_tablet_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-tablet fa-sm mr-1"></i><?= l('create_biolink_cardslider_modal.slider_settings_tabletscreen') ?></label>
		...
		<div class="form-group">
			<label for="<?= 'number_of_sliders_for_desktop_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-desktop fa-sm mr-1"></i><?= l('create_biolink_cardslider_modal.slider_settings_desktopscreen') ?></label>
    ...
    <div id="<?= 'cardslider_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
						...
						<div class="col-7"> 
							<div class="d-flex flex-column">
								<a href="#" data-toggle="collapse" data-target="#<?= 'Slide' . ( $key + 1 ) . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'Slide' . ( $key + 1 )  ?>">
									<strong><?= !empty($item->name) ? $item->name : l('create_biolink_cardslider_modal.slider_title.slider.block') . ' '. ( $key + 1 ) ?></strong>
								</a>
		...
		<div class="collapse" id="<?= 'Slide' . ( $key + 1 ) . $row->biolink_block_id  ?>">	
            <div class="mb-2">
                <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_cardslider_modal.title') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_cardslider_modal.content') ?></label>
    ...
    <div class="mb-3">
        <button style="display:block" data-add="cardslider_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
