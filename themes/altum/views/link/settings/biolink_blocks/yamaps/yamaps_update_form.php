...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="yamaps" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div id="<?= 'yamaps_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
        <?php foreach($row->settings->items as $key => $item): ?>
            <div class="mb-4">
                <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_yamaps_modal.title') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_yamaps_modal.content') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'lat_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-globe fa-sm text-muted mr-1"></i> <?= l('create_biolink_yamaps_modal.lat') ?></label>
            
                    <input id="<?= 'item_lat_' . $key . '_' . $row->biolink_block_id ?>" name="item_lat[<?= $key ?>]" class="form-control" required="required" value="<?= $item->lat ?>" />
                </div>
                
                <div class="form-group">
                    <label for="<?= 'lon_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-globe fa-sm text-muted mr-1"></i> <?= l('create_biolink_yamaps_modal.lon') ?></label>
                    <input id="<?= 'item_lon_' . $key . '_' . $row->biolink_block_id ?>" name="item_lon[<?= $key ?>]" class="form-control" required="required" value="<?= $item->lon ?>" />
                </div>
    ...
    <div class="mb-3">
        <button data-add="yamaps_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
