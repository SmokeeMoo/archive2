...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmprice" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div id="<?= 'tmprice_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
        <?php foreach($row->settings->items as $key => $item): ?>
            <div class="mb-4">
                <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.title') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.content') ?></label>
                    <input id="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_content[<?= $key ?>]" class="form-control" maxlength="2048" value="<?= $item->content ?>" />
                </div>
                
                <div class="form-group">
                    <label for="<?= 'cost_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.cost') ?></label>
                    <input id="<?= 'item_cost_' . $key . '_' . $row->biolink_block_id ?>" name="item_cost[<?= $key ?>]" class="form-control" maxlength="2048" required="required"  value="<?= $item->cost ?>" placeholder="<?= l('create_biolink_tmprice_modal.placeholder') ?>" />
                </div>
    ...
    <div class="mb-3">
                    ...
                    <div class="form-group">
                    <label for="<?= 'tmprice_block_title_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.block_title') ?></label>
                    <input id="<?= 'tmprice_block_title_' . $row->biolink_block_id ?>" type="text" name="block_title" class="form-control" maxlength="2048"  value="<?= $row->settings->block_title ?>" placeholder="<?= l('create_biolink_tmprice_modal.block_title_placeholder') ?>" />
                </div>
                
                    <div class="form-group">
                    <label for="<?= 'tmprice_block_description_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.block_description') ?></label>
                    <input id="<?= 'tmprice_block_description_' . $row->biolink_block_id ?>" type="text" name="block_description" class="form-control" maxlength="2048" value="<?= $row->settings->block_description ?>" placeholder="<?= l('create_biolink_tmprice_modal.block_description_placeholder') ?>" />
                </div>
        
                    <div class="form-group">
                <label for="<?= 'tmprice_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.input.text_color') ?></label>
                <input id="<?= 'tmprice_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                <div class="text_color_pickr"></div>
            </div>
        
                    <div class="form-group">
                <label for="<?= 'tmprice_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmprice_modal.input.background_color') ?></label>
                <input id="<?= 'tmprice_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
                <div class="background_color_pickr"></div>
            </div>
            
            
        <button data-add="tmprice_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
