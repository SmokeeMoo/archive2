...
<form name="update_biolink_" method="post" role="form" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'video_file_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-file fa-sm text-muted mr-1"></i> <?= l('create_biolink_file_modal.file') ?></label>
        <div class="row">
            <div class="col">
        ...
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::array_to_list_format($data->biolink_blocks['video']['whitelisted_file_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->file_size_limit) ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'video_poster_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('create_biolink_video_modal.poster_url') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'video_name_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.name') ?></label>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                ...
                id="<?= 'video_autoplay_' . $row->biolink_block_id ?>"
                name="video_autoplay"
            <?= $row->settings->video_autoplay ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_autoplay_' . $row->biolink_block_id ?>"><?= l('create_biolink_video_modal.video_autoplay') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_controls_' . $row->biolink_block_id ?>"
                name="video_controls"
            <?= $row->settings->video_controls ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_controls_' . $row->biolink_block_id ?>"><?= l('create_biolink_video_modal.video_controls') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_loop_' . $row->biolink_block_id ?>"
                name="video_loop"
            <?= $row->settings->video_loop ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_loop_' . $row->biolink_block_id ?>"><?= l('create_biolink_video_modal.video_loop') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'video_muted_' . $row->biolink_block_id ?>"
                name="video_muted"
            <?= $row->settings->video_muted ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'video_muted_' . $row->biolink_block_id ?>"><?= l('create_biolink_video_modal.video_muted') ?></label>
    </div>
