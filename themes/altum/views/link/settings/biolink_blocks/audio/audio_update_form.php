...
<form name="update_biolink_" method="post" role="form" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'audio_file_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-file fa-sm text-muted mr-1"></i> <?= l('create_biolink_file_modal.file') ?></label>
        <div class="row">
            <div class="col">
        ...
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::array_to_list_format($data->biolink_blocks['audio']['whitelisted_file_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->file_size_limit) ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'audio_name_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.name') ?></label>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                ...
                id="<?= 'audio_autoplay_' . $row->biolink_block_id ?>"
                name="audio_autoplay"
            <?= $row->settings->audio_autoplay ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'audio_autoplay_' . $row->biolink_block_id ?>"><?= l('create_biolink_audio_modal.audio_autoplay') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'audio_controls_' . $row->biolink_block_id ?>"
                name="audio_controls"
            <?= $row->settings->audio_controls ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'audio_controls_' . $row->biolink_block_id ?>"><?= l('create_biolink_audio_modal.audio_controls') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'audio_loop_' . $row->biolink_block_id ?>"
                name="audio_loop"
            <?= $row->settings->audio_loop ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'audio_loop_' . $row->biolink_block_id ?>"><?= l('create_biolink_audio_modal.audio_loop') ?></label>
    </div>
    ...
    <div class="form-group custom-control custom-switch">
        <input
                type="checkbox"
                class="custom-control-input"
                id="<?= 'audio_muted_' . $row->biolink_block_id ?>"
                name="audio_muted"
            <?= $row->settings->audio_muted ? 'checked="checked"' : null ?>
        >
        <label class="custom-control-label" for="<?= 'audio_muted_' . $row->biolink_block_id ?>"><?= l('create_biolink_audio_modal.audio_muted') ?></label>
    </div>
