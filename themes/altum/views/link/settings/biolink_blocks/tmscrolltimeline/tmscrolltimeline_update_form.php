...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmscrolltimeline" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>
    
    <div id="<?= 'tmscrolltimeline_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
        <?php foreach($row->settings->items as $key => $item): ?>
         <div class="mb-4">
             
             <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.title') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.text') ?></label>
                    <textarea id="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_content[<?= $key ?>]" class="form-control" maxlength="2048" required="required"><?= $item->content ?></textarea>
                </div>
                
                 <div class="form-group">
                    <label for="<?= 'item_date_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-clock fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.date') ?></label>
                     <input id="<?= 'item_date_' . $key . '_' . $row->biolink_block_id ?>" type="text" name="item_date[<?= $key ?>]" class="form-control" value="<?= $item->date ?>" required="required" />
                                ...
                                <div class="form-group">
                    <label for="<?= 'item_icon_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-expand fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.icon') ?></label>
                    <select id="<?= 'icon_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_icon[<?= $key ?>]" class="form-control" >
            <option value="warning" <?= $item->icon == 'warning' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmscrolltimeline_modal.warning') ?></option>
            <option value="danger" <?= $item->icon == 'danger' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmscrolltimeline_modal.danger') ?></option>
            <option value="primary" <?= $item->icon == 'primary' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmscrolltimeline_modal.primary') ?></option>
            <option value="success" <?= $item->icon == 'success' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmscrolltimeline_modal.success') ?></option>
    ...
    <div class="mb-3">
                    ...
                    <div class="form-group">
                    <label for="<?= 'tmscrolltimeline_title_block_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.block_title') ?></label>
                    <input id="<?= 'tmscrolltimeline_title_block_' . $row->biolink_block_id ?>" type="text" name="title_block" class="form-control" maxlength="2048"  value="<?= $row->settings->title_block ?>" placeholder="<?= l('create_biolink_tmscrolltimeline_modal.block_title_placeholder') ?>" />
                </div>
    
                    <div class="form-group">
                <label for="<?= 'tmscrolltimeline_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.input.text_color') ?></label>
                <input id="<?= 'tmscrolltimeline_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                <div class="text_color_pickr"></div>
            </div>
        
                    <div class="form-group">
                <label for="<?= 'tmscrolltimeline_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmscrolltimeline_modal.input.background_color') ?></label>
                <input id="<?= 'tmscrolltimeline_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
                <div class="background_color_pickr"></div>
            </div>
            
            
        <button data-add="tmscrolltimeline_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
