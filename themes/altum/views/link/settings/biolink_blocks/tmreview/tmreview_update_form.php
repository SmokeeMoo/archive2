...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmreview" />
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#setting" aria-expanded="false" aria-controls="setting">
		...
		<?= l('create_biolink_tmreview_modal.tmreview_settings') ?>
	...
	<div class="collapse" id="setting">	
		 ...
		 <div class="form-group">
                <label for="<?= 'tmreview_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmreview_modal.input.text_color') ?></label>
                <input id="<?= 'tmreview_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                <div class="text_color_pickr"></div>
            </div>
        
                    <div class="form-group">
                <label for="<?= 'tmreview_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmreview_modal.input.background_color') ?></label>
                <input id="<?= 'tmreview_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
    ...
    <div id="<?= 'tmreview_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
				...
				<?php if($item->enable): ?>
					<div class="d-flex align-items-center "style="border-radius: 6px; padding: 10px; border: 2px solid #d3d3d35e;">	
						<div class="col-2 mr-2 p-0"> 
							<img src="<?= $item->image ? UPLOADS_FULL_URL . 'tmreview_images/' . $item->image : null ?>" class="img-fluid rounded <?= !empty($item->image) ? null : 'd-none' ?>" loading="lazy" />   
						</div>
						<div class="col-5"> 
							<div class="d-flex flex-column">
								<a href="#" data-toggle="collapse" data-target="#<?= 'Review' . ( $key + 1 ) . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'Review' . ( $key + 1 )  ?>">
									<strong><?= !empty($item->name) ? $item->name : l('create_biolink_tmreview_modal.tmreview_title.tmreview.block') . ' '. ( $key + 1 ) ?></strong>
								</a>
					...
					<!--<div class="col-3 col-md-2 d-flex justify-content-end">
							<a href="#" data-toggle="collapse" data-target="#<?= 'Review' . ( $key + 1 ). $row->biolink_block_id  ?>" aria-expanded="false" aria-controls="<?= 'Review' . ( $key + 1 ) . $row->biolink_block_id ?>" >
		...
		<div class="collapse" id="<?= 'Review' . ( $key + 1 ) . $row->biolink_block_id  ?>">	
            <div class="mb-2">
               <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmreview_modal.title') ?></label>
                    <input id="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>" type="text" name="item_title[<?= $key ?>]" class="form-control" value="<?= $item->title ?>" required="required" placeholder="<?= l('create_biolink_tmreview_modal.title.placeholder') ?>"/>
                </div>
                
                <div class="form-group">
                    <label for="<?= 'item_subcontent_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmreview_modal.subcontent') ?></label>
                    <input id="<?= 'item_subcontent_' . $key . '_' . $row->biolink_block_id ?>" type="text" name="item_subcontent[<?= $key ?>]" class="form-control" value="<?= $item->subcontent ?>" required="required" placeholder="<?= l('create_biolink_tmreview_modal.subcontent.placeholder') ?>"/>
                </div>

                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmreview_modal.content') ?></label>
                    <textarea id="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_content[<?= $key ?>]" class="form-control" required="required" placeholder="<?= l('create_biolink_tmreview_modal.content.placeholder') ?>"><?= $item->content ?></textarea>
                </div>
					...
					<div class="form-group">
						...
						<input id="<?= 'item_image_' . $key . '_' . $row->biolink_block_id ?>" type="file" name="<?= 'image' . $key ?>" accept=".gif, .png, .jpg, .jpeg, .svg" class="form-control-file"/>
    ...
    <div class="mb-3">
        <button style="display:block" data-add="tmreview_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
