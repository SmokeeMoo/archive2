...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmtextmorph" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div id="<?= 'tmtextmorph_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
        <?php foreach($row->settings->items as $key => $item): ?>
            <div class="mb-4">
                <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmtextmorph_modal.title') ?></label>
        ...
        <?php endforeach ?>
            ...
            <button data-add="tmtextmorph_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    ...
    <div class="mb-3">
        ...
        <div class="collapse" id="<?= 'button_settings_container_' . $row->biolink_block_id ?>">
                    ...
                    <div class="row btn-group-toggle" data-toggle="buttons">
                        <?php foreach(['center', 'left', 'right'] as $text_alignment): ?>
                            <div class="col-4">
                    ...
                    <div class="form-group">
                <label for="<?= 'tmtextmorph_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmtextmorph_modal.input.text_color') ?></label>
                <input id="<?= 'tmtextmorph_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
                <div class="text_color_pickr"></div>
            </div>
            
            
            <div class="form-group">
			<label for="<?= 'font_size_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-font fa-sm mr-1"></i><?= l('create_biolink_textmorph_modal.font_size') ?></label>
			<input id="<?= 'font_size_' . $row->biolink_block_id ?>" type="number" min="1" max="8"   name="font_size" class="form-control" value="<?= $row->settings->font_size ?>" maxlength="128" placeholder="4" />
		</div>
		
		    <div class="form-group">
			<label for="<?= 'speed_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-bolt fa-sm mr-1"></i><?= l('create_biolink_textmorph_modal.speed') ?></label>
			<input id="<?= 'speed_' . $row->biolink_block_id ?>" type="number" min="1" max="6"   name="speed" class="form-control" value="<?= $row->settings->speed ?>" maxlength="128" placeholder="2" />
		</div>
