...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmnotification" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>
    
    <div id="<?= 'tmnotification_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
        <?php foreach($row->settings->items as $key => $item): ?>
         <div class="mb-4">
             
             <div class="form-group">
                    <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmnotification_modal.title') ?></label>
                ...
                <div class="form-group">
                    <label for="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmnotification_modal.text') ?></label>
                    <textarea id="<?= 'item_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_content[<?= $key ?>]" class="form-control" maxlength="2048" required="required"><?= $item->content ?></textarea>
                                ...
                                <div class="form-group">
                    <label for="<?= 'item_icon_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-expand fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmnotification_modal.icon') ?></label>
                    <select id="<?= 'icon_content_' . $key . '_' . $row->biolink_block_id ?>" name="item_icon[<?= $key ?>]" class="form-control" >
            <option value="error" <?= $item->icon == 'error' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.error') ?></option>
            <option value="warning" <?= $item->icon == 'warning' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.warning') ?></option>
            <option value="success" <?= $item->icon == 'success' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.success') ?></option>
            <option value="message" <?= $item->icon == 'message' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.message') ?></option>
            <option value="clock" <?= $item->icon == 'clock' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.clock') ?></option>
            <option value="up" <?= $item->icon == 'up' ? 'selected="selected"' : null ?>><?= l('create_biolink_tmnotification_modal.up') ?></option>
    ...
    <div class="mb-3">
        ...
        <button data-add="tmnotification_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
