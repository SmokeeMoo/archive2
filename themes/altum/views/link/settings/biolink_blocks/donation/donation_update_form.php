...
<form name="update_biolink_" method="post" role="form">
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'donation_settings_container_' . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'donation_settings_container_' . $row->biolink_block_id ?>">
        <i class="fas fa-fw fa-hand-holding-dollar fa-sm mr-1"></i> <?= l('create_biolink_donation_modal.donation_header') ?>
    </button>
    ...
    <div class="collapse" id="<?= 'donation_settings_container_' . $row->biolink_block_id ?>">
        ...
        <div class="form-group">
            <label for="<?= 'donation_thank_you_url_' . $row->biolink_block_id ?>"><?= l('create_biolink_donation_modal.thank_you_url') ?></label>
            <input type="url" id="<?= 'donation_thank_you_url_' . $row->biolink_block_id ?>" name="thank_you_url" class="form-control" value="<?= $row->settings->thank_you_url ?? null ?>" maxlength="<?= $data->biolink_blocks['product']['fields']['thank_you_url']['max_length'] ?>" />
    ...
    <div class="collapse" id="<?= 'button_settings_container_' . $row->biolink_block_id ?>">
        <div class="form-group">
            <label for="<?= 'donation_name_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.name') ?></label>
        ...
        <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->thumbnail_image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?>">
            <label for="<?= 'donation_image_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.image') ?></label>
            <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
                'id'=> 'donation_image_' . $row->biolink_block_id,
                'uploads_file_key' => 'block_thumbnail_images',
                'file_key' => 'image',
                'already_existing_image' => $row->settings->image,
                'image_container' => 'image',
                'accept' => \Altum\Uploads::array_to_list_format($data->biolink_blocks['donation']['whitelisted_thumbnail_image_extensions']),
            ]) ?>
        ...
        <div class="form-group">
            <label for="<?= 'donation_icon_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-icons fa-sm text-muted mr-1"></i> <?= l('global.icon') ?></label>
            <input id="<?= 'donation_icon_' . $row->biolink_block_id ?>" type="text" name="icon" class="form-control" value="<?= $row->settings->icon ?>" placeholder="<?= l('global.icon_placeholder') ?>" />
            <small class="form-text text-muted"><?= l('global.icon_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'donation_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.text_color') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'donation_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.background_color') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'donation_animation_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-film fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.animation') ?></label>
            <select id="<?= 'donation_animation_' . $row->biolink_block_id ?>" name="animation" class="custom-select">
        ...
        <div class="form-group" data-animation="<?= implode(',', require APP_PATH . 'includes/biolink_animations.php') ?>">
            <label for="<?= 'donation_animation_runs_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-play-circle fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_modal.input.animation_runs') ?></label>
            <select id="<?= 'donation_animation_runs_' . $row->biolink_block_id ?>" name="animation_runs" class="custom-select">
