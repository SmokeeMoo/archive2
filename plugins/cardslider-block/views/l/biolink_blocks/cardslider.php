...
<div class="cardslider">
    ...
    <?php foreach($data->link->settings->items as $key => $item): ?>
     <?php if($item->enable): ?>
      <div style="display:block;background: <?= $data->link->settings->background_color ?>;border-radius: 10px; color: <?= $data->link->settings->text_color ?>; line-height: 1.1; padding: 20px 25px;    margin: 0 5px;"><div style="font-weight:bold; margin: 10px 0 15px; font-size: 120%"><?= $item->title ?></div>
      <p><?= $item->content ?></p>
      </div>
      <?php endif ?>
 <?php endforeach ?>
